...
class _CameraExampleHomeState extends State<CameraExampleHome>
                    ...
                    children: [
                      ...
                      if (location != null)
                        Align(
                          ...
                          child: Text('Accuracy:${location!.latitude}'),
                        ),
                      if (location != null)
                        Align(
                            alignment: Alignment.bottomLeft,
  ...
  Widget directionvale() {
    ...
    return Row(
      children: [
        if (val == 0) Text('N'),
        if (val > 0 && val < 90) Text('NE'),
        if (val == 90) Text('E'),
        if (val > 90 && val < 180) Text('ES'),
        if (val == 180) Text('S'),
        if (val > 180 && val < 270) Text('SW'),
        if (val == 270) Text('W'),
        if (val > 270 && val < 360) Text('WN'),
        SizedBox(
          width: 20,
        ),
                  ...
                  actions: [
                    ...
                    ElevatedButton(
                        onPressed: () {
                          ...
                          saveimage(file, textcontroller.text, path,
                              directionval, trackname.text);
                          final snackBar = SnackBar(
                              content: Text(
                                  'Image Saved In App Galley as ${textcontroller.text}.png'));
  ...
  void saveimage(XFile file, String name, Directory path, directionval,
      String tracknam) async {
    ...
    screenshotController
      ...
      final imagePath = File('${path.path}/images/${textcontroller.text}.png');
      bool fileExists = await imagePath.exists();
      if (fileExists) {
        Flushbar(
          backgroundColor: Colors.red,
          message: 'Please Change name image with same name already exist',
      ...
      DatabaseHelper.instance.addtrackmedia(
          '${textcontroller.text}.png', 'image', location!, tracknam);
      textcontroller.clear();
      Flushbar(
        backgroundColor: Colors.red,
        message: 'Media Saved in track gallery',
        duration: Duration(seconds: 3),
      ).show(context);
    });
  }
                ...
                actions: [
                  ...
                  ElevatedButton(
                      onPressed: () async {
                        ...
                        Navigator.of(context).pop();
                        processvideo(
                            file, path, textcontroller.text, trackname.text);
                      },
                      child: Text('Save'))
                ],
              );
  ...
  Future processvideo(
      XFile file, Directory path, String name, String tracknam) async {
    try {
      final tapiocaBalls = [
      ...
      Timer(Duration(seconds: 2), () {
        cup.suckUp('${path.path}/videos/$name.mp4').then((_) {
          DatabaseHelper.instance.addtrackmedia(
              '${textcontroller.text}.mp4', 'video', location!, tracknam);
          textcontroller.clear();
          setState(() {
            loader = false;
          });
